...
summary-on-network:
    ...
    echo "       GNOSISVPN_CONFIG_PATH=${bundle_dir}/client-on-network.toml \\"
    echo "       GNOSISVPN_HOPR_BLOKLI_URL=\"${blokli_url}\" \\"
    echo "       GNOSISVPN_HOPR_IDENTITY_FILE=${bundle_dir}/extra_id.id \\"
    echo "       GNOSISVPN_HOPR_IDENTITY_PASS=\"\$(cat ${bundle_dir}/extra_id.password)\" \\"
    echo "       GNOSISVPN_WORKER_USER={{CLIENT_WORKER_USER}} \\"
    echo "       GNOSISVPN_CLIENT_AUTOSTART=30min \\"
    echo "       ./result/bin/gnosis_vpn-root --worker-binary ./result/bin/gnosis_vpn-worker"
    echo ""
